...
public class TowerComponent extends Canvas implements Runnable, MouseListener, MouseMotionListener, KeyListener
{
    ...
    private void init()
    {
        ...
        {
            bitmaps.loadAll();
            bitmaps.startWatching(); // Start auto-reload file watcher
        }
        catch (IOException e)
        {
            e.printStackTrace();
    ...
    private void reloadSprites()
    {
        try
        {
            System.out.println("Reloading sprites...");
            bitmaps.loadAll();
            System.out.println("Sprites reloaded!");
        }
        catch (IOException e)
        {
            System.err.println("Failed to reload sprites: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    {
        // Enable hot-reload: load sprites from res/ folder if it exists
        java.io.File resDir = new java.io.File("res");
        if (resDir.exists() && resDir.isDirectory())
        {
            Bitmaps.setResPath("res");
            System.out.println("Hot-reload enabled: sprites auto-reload when res/*.gif files change (F5 for manual reload)");
        }
