...
must_haves:
  truths:
    - "Pathfinder finds route through walkable terrain avoiding obstacles"
    - "Pathfinder reports when no route exists (blocked terrain)"
    - "Diagonal paths are shorter than cardinal-only paths when going corner-to-corner"
    - "Same start/goal always produces identical path (deterministic)"
  artifacts:
