...
Add import: `import com.mojang.tower.pathfinding.*;`

**Modify tick() method:**

Current behavior (around line 118-130):
- When `wanderTime == 0 && job != null && job.hasTarget()`, Peon moves directly toward target
- When blocked, random rotation and wanderTime is set

New behavior:
1. **When job has target but no path:** Request path from PathfindingService
2. **When path exists:** Move toward current waypoint instead of direct target
3. **When waypoint reached:** Advance to next waypoint
4. **When blocked mid-path:** Clear path and fall back to existing random behavior
5. **When path complete:** Check if at job target, call job.arrived() if so

Replace the movement calculation section (approximately lines 117-160) with:
...
if (wanderTime == 0 && job != null && job.hasTarget()) {
    ...
    if (xd * xd + yd * yd < rd * rd) {
        ...
        if (currentPath == null) {
            ...
            switch (result) {
                ...
                case PathResult.NotFound(var reason) -> {
                    // No path found - use random movement
                    currentPath = null;
                }
            }
        }
        ...
        if (currentPath != null && pathIndex < currentPath.size()) {
            ...
            // No path available - random direction
...
**Also update setJob() to clear path when job changes:**
```java
...
Compile and run golden master test: `cd /home/docinhos/Documents/Java/BreakingTheTower && mvn test -Dtest=GoldenMasterTest -q`
  ...
  <done>Peon uses computed paths to navigate; golden master test passes</done>
</task>
...
Tests pass: `cd /home/docinhos/Documents/Java/BreakingTheTower && mvn test -Dtest=PathfindingServiceTest -q`
  ...
  <done>PathfindingService integration tests pass confirming coordinate conversion works</done>
</task>
...
5. Manual verification (optional): Run game, click target behind rock, observe peon pathfinding around it
</verification>
